...
### Linear probing and Context Optimization (CoOp) comparison
To further evaluate representation quality, we also perform **Context Optimization (CoOp), CLS Few-Shot Linear probing and Video Few/Full-shot linear probing**.  
The figure below shows CoOp (top) and CLS Linear Probing (middle) curves are plotted over the number of labeled training examples per class (1, 2, 4, 8, 16). Video Linear Probing (bottom) reports performance using (10%, 50%, 100%) of the available labeled training videos in each dataset. Compared models include SurgCLIP, CLIP, MedSigLIP, PeskaVLP, and LEMON. Bottom tables report the number of training videos corresponding to each percentage for each dataset.
